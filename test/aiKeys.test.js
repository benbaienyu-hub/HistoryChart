...
describe('secretBox', () => {
  ...
  it('produces different ciphertext each time, so equal keys are not obvious', () => {
  ...
  it('returns null for a box it cannot understand', () => {
    expect(open(null)).toBeNull();
    expect(open({})).toBeNull();
    expect(open({ v: 99, iv: 'x', tag: 'y', data: 'z' })).toBeNull();
  });

  it('keeps the key file to the owner only', () => {
    seal('anything');
  ...
  it('cannot open a row sealed under a different key', () => {
    const box = seal(KEY);
  ...
  it('prefers LACUNA_SECRET over the key file when set', () => {
...
describe('storing a key for an account', () => {
  it('keeps the key out of the database file', () => {
    setUserAiKey('user-1', { apiKey: KEY });
  ...
  it('gives it back to the one caller that needs it', () => {
    setUserAiKey('user-1', { apiKey: KEY, baseUrl: 'https://api.groq.com/openai/v1', model: 'llama' });
    expect(getUserAiCredentials('user-1')).toEqual({
      apiKey: KEY,
  ...
  it('describes it without revealing it', () => {
    setUserAiKey('user-1', { apiKey: KEY });
    const described = describeUserAiKey('user-1');
  ...
  it('reports an account with no key of its own', () => {
    expect(describeUserAiKey('nobody')).toMatchObject({ configured: false, preview: null });
    expect(getUserAiCredentials('nobody')).toBeNull();
  });

  it('keeps one row per account', () => {
    setUserAiKey('user-1', { apiKey: KEY });
    setUserAiKey('user-1', { apiKey: `${KEY}-second` });
    expect(readDb().aiKeys).toHaveLength(1);
    expect(getUserAiCredentials('user-1').apiKey).toBe(`${KEY}-second`);
  });

  it('does not mix up two accounts', () => {
    setUserAiKey('user-1', { apiKey: 'sk-one-one-one-one-one' });
    setUserAiKey('user-2', { apiKey: 'sk-two-two-two-two-two' });
    expect(getUserAiCredentials('user-1').apiKey).toBe('sk-one-one-one-one-one');
    expect(getUserAiCredentials('user-2').apiKey).toBe('sk-two-two-two-two-two');
  });

  it('changes the provider and model without being re-sent the key', () => {
    // The browser was never given the key, so it cannot send it back — a partial
    // update has to be able to keep what is stored.
    setUserAiKey('user-1', { apiKey: KEY, model: 'gpt-4o' });
    updateUserAiSettings('user-1', { baseUrl: 'https://api.groq.com/openai/v1', model: 'llama' });
    expect(getUserAiCredentials('user-1')).toEqual({
      apiKey: KEY,
      baseUrl: 'https://api.groq.com/openai/v1',
      model: 'llama',
  ...
  it('has nothing to update for an account with no key', () => {
    expect(updateUserAiSettings('nobody', { model: 'x' })).toBeNull();
  });

  it('forgets the key when asked', () => {
    setUserAiKey('user-1', { apiKey: KEY });
    clearUserAiKey('user-1');
    expect(getUserAiCredentials('user-1')).toBeNull();
    expect(readDb().aiKeys).toHaveLength(0);
  });

  it('says so when the row cannot be decrypted, rather than claiming a working key', () => {
    // What a database restored without its secret.key looks like.
    setUserAiKey('user-1', { apiKey: KEY });
    rmSync(join(dir, 'secret.key'));
    resetSecretCacheForTests();
    expect(describeUserAiKey('user-1')).toMatchObject({ configured: true, unreadable: true });
    expect(getUserAiCredentials('user-1')).toBeNull();
  });

  it('normalizes a trailing slash off the provider URL', () => {
    setUserAiKey('user-1', { apiKey: KEY, baseUrl: 'https://api.groq.com/openai/v1//' });
    expect(getUserAiCredentials('user-1').baseUrl).toBe('https://api.groq.com/openai/v1');
  });

  it('treats blank provider and model as unset rather than empty strings', () => {
    setUserAiKey('user-1', { apiKey: KEY, baseUrl: '   ', model: '  ' });
    expect(getUserAiCredentials('user-1')).toEqual({ apiKey: KEY, baseUrl: null, model: null });
  });
});

describe('what we refuse to store', () => {
  it('names the problem with a key that cannot be right', () => {
  ...
  it('accepts an https provider, and localhost for a local model', () => {
  ...
  it('refuses plain http to somewhere else, which would put the key on the wire', () => {
...
describe('maskKey', () => {
  ...
  it('does not reveal a short value by trying to mask it', () => {
