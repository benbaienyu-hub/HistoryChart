  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-6 backdrop-blur-sm">
      <motion.div
        ...
        className="w-full max-w-[400px] rounded-3xl border border-line bg-panel p-6 shadow-[0_24px_64px_-16px_rgba(0,0,0,0.3)]"
      >
        ...
        <form onSubmit={save} className="mt-4">
          <input
            ...
            className="w-full rounded-xl border border-line2 bg-panel px-3 py-2 text-[13.5px] text-ink placeholder:text-subink/60 focus:border-accent/50 focus:outline-none focus:ring-2 focus:ring-accent/15"
          />
              ...
              <button
                ...
                className={`rounded-full border px-2.5 py-1 text-[11.5px] transition-colors ${
                  value === p
                    ...
                    : 'border-line2 text-subink hover:bg-sunken hover:text-ink'
                }`}
              >
                {p}
              </button>
          ...
          <div className="mt-5 flex items-center justify-end gap-2">
            {onDelete && (
              <button
                type="button"
                onClick={onDelete}
                className="mr-auto rounded-xl px-3 py-2 text-[13px] text-subink hover:bg-danger-bg hover:text-danger"
              >
                Remove
              </button>
            )}
